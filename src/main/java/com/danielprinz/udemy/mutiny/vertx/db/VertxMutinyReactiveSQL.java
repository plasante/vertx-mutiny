...
public class VertxMutinyReactiveSQL extends AbstractVerticle {
  ...
  private Uni<JsonArray> executeQuery(Pool db) {
    ...
    return getJsonArrayUni(db, "users");
  }

  /**
   * Transform a database RowSet into a Uni<JsonArray>
   * @param db
   * @param tableName
   * @return
   */
  private static Uni<JsonArray> getJsonArrayUni(Pool db, String tableName) {
    if (tableName == null || tableName.isEmpty()) {
      LOG.error("Table name is null or empty. Returning empty array.");
      return Uni.createFrom().item(new JsonArray());
    }
    return db.query("SELECT * from " + tableName)
      .execute()
      .onItem().transform(rows -> {
        var data = new JsonArray();   // we want to return this in the HTTP response
        for (Row row : rows) {        // Transform Rowset into JsonArray
          data.add(row.toJson());
        }
